...
`urlPrefixes` in `registry.json` scopes each work by page. The root pages
(LactMed `NBK501922`, LiverTox `NBK547852`, StatPearls `NBK430685`) are
claimed by their sources. Every other Bookshelf page resolves to
`ncbi-bookshelf`, a per-item source: its licence has to be checked page by
page. Bookshelf ids do not encode which work a page belongs to and the
...
Open each id below and confirm it belongs to the work claimed. When it
does, add `ncbi.nlm.nih.gov/books/nbkNNNNNN` to that source's
`urlPrefixes` (LactMed and LiverTox are already `embeddable: yes` as works).
Anything that turns out to be StatPearls or another work: leave it off,
and correct the citation's label in the bundle.
