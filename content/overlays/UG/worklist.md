# Uganda (UG) — content authoring worklist

_Generated 2026-06-12 by scripts/ingest-country-content.ts. Licence-gated:
...
Uganda currently serves WHO/Kenya-default reference content with a "not yet
localized" label. Authoring the clinical domains below + clinical sign-off
(overlay.json `signedOff:true`) flips Uganda to **localized**.

## Locale (authored)
- Patient-facing languages: en, sw
- Official languages: en, sw
- National formulary source: `moh-ug` (cite-only)
...
- **Uganda Clinical Guidelines + Essential Medicines List (UCG/EMHSLU)** (Uganda Ministry of Health) — `cite-only`, Government of Uganda copyright
...
## Sign-off checklist
- [ ] National overlay records authored & jurisdiction-tagged `UG`
