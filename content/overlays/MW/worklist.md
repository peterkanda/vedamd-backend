# Malawi (MW) — content authoring worklist
...
Malawi currently serves WHO/Kenya-default reference content with a "not yet
localized" label. Authoring the clinical domains below + clinical sign-off
(overlay.json `signedOff:true`) flips Malawi to **localized**.

## Locale (authored)
- Patient-facing languages: en, ny
- Official languages: en, ny
- National formulary source: `moh-mw` (cite-only)
...
- **Malawi Standard Treatment Guidelines + EML** (Malawi Ministry of Health) — `cite-only`, Government of Malawi copyright
...
## Sign-off checklist
- [ ] National overlay records authored & jurisdiction-tagged `MW`
