# Rwanda (RW) — content authoring worklist
...
Rwanda currently serves WHO/Kenya-default reference content with a "not yet
localized" label. Authoring the clinical domains below + clinical sign-off
(overlay.json `signedOff:true`) flips Rwanda to **localized**.

## Locale (authored)
- Patient-facing languages: en, rw
- Official languages: rw, en, fr, sw
- National formulary source: `moh-rw` (cite-only)
...
- **Rwanda clinical treatment guidelines** (Rwanda Ministry of Health) — `cite-only`, Government of Rwanda copyright
...
## Sign-off checklist
- [ ] National overlay records authored & jurisdiction-tagged `RW`
