...
export function calculateDose(drug: DrugRecord, inputs: DosingInput): DosingResult {
  ...
  if (renalHit?.caution) {
    baseResult.warnings = [...baseResult.warnings, `Renal: ${renalHit.adjustment}`];
  }
...
export function matchRenal(
  ...
  for (const band of bands) {
    ...
    // crClMaxExclusive is set at load from the drug's authoring convention.
    // Without it, read the bound inclusively: the data documents inclusive
    // bounds, and missing a boundary CrCl is the worse error.
    const okUpper =
      band.crClMaxExclusive !== undefined
        ? crClMlMin < band.crClMaxExclusive
        : band.crClMaxMlMin === undefined || crClMlMin <= band.crClMaxMlMin;
