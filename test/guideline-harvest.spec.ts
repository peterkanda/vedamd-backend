 ...
 *
 * No MoH source is embeddable wholesale: each is cite-only, or (Kenya)
 * checked document by document because a few documents allow
 * non-commercial reproduction. content/sources/README.md says to cite and
 * link, author original logic from the underlying clinical facts, and
 * secure permission where logic derives from national guidelines. A future change that starts storing
...
describe('guideline harvester stays within the MoH licence', () => {
  ...
  it('points every country at a registered MoH source that is never embeddable wholesale', () => {
    for (const c of SOURCES.countries) {
      ...
      expect(src!.embeddable).not.toBe('yes');
