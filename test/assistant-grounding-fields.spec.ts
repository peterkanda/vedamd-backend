...
import { annotate } from '../src/modules/assistant/assistant.service';
import { summarizeRecord } from '../src/modules/knowledge/grounding/record-summary';
 ...
 * the question.
 *
 * The grounding summariser once used an allow-list of fields written around
 ...
 * record. These tests pin the fields that make each domain's answer, through
 * summarizeRecord — the summariser the chat assistant grounds on.
 */
const BUDGET = 1500; // AssistantService's per-record budget

function summarize(rec: Record<string, unknown>, domain = ''): string {
  return summarizeRecord(rec, domain, { budget: BUDGET }).text;
}
...
function summarizeFor(domain: string, rec: Record<string, unknown>): string {
  return summarize(annotate(domain, rec), domain);
}
